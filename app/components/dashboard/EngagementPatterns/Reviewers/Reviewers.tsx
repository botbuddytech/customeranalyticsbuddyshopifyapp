...
function getDescription(dateRange: string): string {
  switch (dateRange) {
    case "today": return "Reviewers today";
    case "yesterday": return "Reviewers yesterday";
    case "7days": case "last7Days": return "Reviewers in the last 7 days";
    case "30days": case "last30Days": return "Reviewers in the last 30 days";
    case "90days": case "last90Days": return "Reviewers in the last 90 days";
    case "thisMonth": return "Reviewers this month";
    case "lastMonth": return "Reviewers last month";
    default: return "Reviewers in the selected period";
  }
}
...
export function Reviewers({
  ...
  const fetcher = useFetcher<ReviewersData>();
  const customersListFetcher = useFetcher<CustomersListData>();
  const [data, setData] = useState<ReviewersData | null>(null);
  ...
  useEffect(() => {
    ...
    fetcher.load(
      `/api/dashboard/engagement-patterns/reviewers?dateRange=${dateRange}`,
    );
  }, [dateRange]);
  ...
  const handleViewSegment = (segmentName: string) => {
    if (segmentName === "Reviewers") {
      setShowCustomersModal(true);
      customersListFetcher.load(
        `/api/dashboard/engagement-patterns/reviewers/list?dateRange=${dateRange}`,
      );
  ...
  const handleExportData = () => {
    ...
    link.setAttribute(
      "download",
      `reviewers-${getDateRangeLabel(dateRange).replace(/\s+/g, "-")}-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={`Reviewers - ${getDateRangeLabel(dateRange)}`}
        primaryAction={{
          content: "Close",
            ...
            {customersListFetcher.state === "loading" ? (
              ...
              <>
                <Text as="p" variant="bodyMd">
                  ...
                  {customersListFetcher.data.total !== 1 ? "s" : ""} who submitted reviews for{" "}
              ...
              <Text as="p" variant="bodyMd">
                No reviewers found for {getDateRangeLabel(dateRange)}.
              </Text>
            )}
          </BlockStack>
        </Modal.Section>
