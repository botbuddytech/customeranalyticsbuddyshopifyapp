...
function getDescription(dateRange: string): string {
  switch (dateRange) {
    case "today": return "Abandoned carts today";
    case "yesterday": return "Abandoned carts yesterday";
    case "7days": case "last7Days": return "Abandoned carts in the last 7 days";
    case "30days": case "last30Days": return "Abandoned carts in the last 30 days";
    case "90days": case "last90Days": return "Abandoned carts in the last 90 days";
    case "thisMonth": return "Abandoned carts this month";
    case "lastMonth": return "Abandoned carts last month";
    default: return "Abandoned carts in the selected period";
  }
}

function getStatusFromGrowth(growth: number): "success" | "warning" | "critical" {
  if (growth === 0) return "warning";
  // For abandoned carts: decrease is good, increase is bad
  if (growth < 0) {
...
export function AbandonedCarts({
  ...
  const fetcher = useFetcher<AbandonedCartsData>();
  const ordersListFetcher = useFetcher<OrdersListData>();
  const [data, setData] = useState<AbandonedCartsData | null>(null);
  ...
  useEffect(() => {
    ...
    fetcher.load(
      `/api/dashboard/purchase-order-behavior/abandoned-carts?dateRange=${dateRange}`,
    );
  }, [dateRange]);
  ...
  const chartData =
    data.dataPoints && data.dataPoints.length > 1
      ? (() => {
          ...
          const startCount = startPoint.count;
          const endCount = endPoint.count;
          // For abandoned carts: decrease is good (green), increase is bad (red)
  ...
  const handleViewSegment = (segmentName: string) => {
    if (segmentName === "Abandoned Carts") {
      setShowOrdersModal(true);
      ordersListFetcher.load(
        `/api/dashboard/purchase-order-behavior/abandoned-carts/list?dateRange=${dateRange}`,
      );
  ...
  const handleExportData = () => {
    ...
    const headers = ["Order Number", "Customer Name", "Customer Email", "Date", "Total"];
    ...
    link.setAttribute(
      "download",
      `abandoned-carts-${getDateRangeLabel(dateRange).replace(/\s+/g, "-")}-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={`Abandoned Carts - ${getDateRangeLabel(dateRange)}`}
        primaryAction={{
          content: "Close",
            ...
            {ordersListFetcher.state === "loading" ? (
              ...
              <>
                <Text as="p" variant="bodyMd">
                  Showing {ordersListFetcher.data.total} abandoned cart
              ...
              <Text as="p" variant="bodyMd">
                No abandoned carts found for {getDateRangeLabel(dateRange)}.
              </Text>
            )}
          </BlockStack>
        </Modal.Section>
