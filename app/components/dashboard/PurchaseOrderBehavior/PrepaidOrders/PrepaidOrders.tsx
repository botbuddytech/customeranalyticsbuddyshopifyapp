...
interface PrepaidOrdersData {
  count: number;
  dataPoints: Array<{ date: string; count: number }>;
  error?: string;
}

interface PrepaidOrdersProps {
  dateRange?: string;
...
function getDescription(dateRange: string): string {
  switch (dateRange) {
    case "today": return "Prepaid orders today";
    case "yesterday": return "Prepaid orders yesterday";
    case "7days": case "last7Days": return "Prepaid orders in the last 7 days";
    case "30days": case "last30Days": return "Prepaid orders in the last 30 days";
    case "90days": case "last90Days": return "Prepaid orders in the last 90 days";
    case "thisMonth": return "Prepaid orders this month";
    case "lastMonth": return "Prepaid orders last month";
    default: return "Prepaid orders in the selected period";
  }
}
...
export function PrepaidOrders({
  dateRange = "30days",
  onViewSegment,
}: PrepaidOrdersProps) {
  const fetcher = useFetcher<PrepaidOrdersData>();
  const ordersListFetcher = useFetcher<OrdersListData>();
  const [data, setData] = useState<PrepaidOrdersData | null>(null);
  ...
  useEffect(() => {
    ...
    fetcher.load(
      `/api/dashboard/purchase-order-behavior/prepaid-orders?dateRange=${dateRange}`,
    );
  }, [dateRange]);
  ...
  const handleViewSegment = (segmentName: string) => {
    if (segmentName === "Prepaid Orders") {
      setShowOrdersModal(true);
      ordersListFetcher.load(
        `/api/dashboard/purchase-order-behavior/prepaid-orders/list?dateRange=${dateRange}`,
      );
  ...
  const handleExportData = () => {
    ...
    link.setAttribute(
      "download",
      `prepaid-orders-${getDateRangeLabel(dateRange).replace(/\s+/g, "-")}-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={`Prepaid Orders - ${getDateRangeLabel(dateRange)}`}
        primaryAction={{
          content: "Close",
            ...
            {ordersListFetcher.state === "loading" ? (
              ...
              <>
                <Text as="p" variant="bodyMd">
                  Showing {ordersListFetcher.data.total} prepaid order
              ...
              <Text as="p" variant="bodyMd">
                No prepaid orders found for {getDateRangeLabel(dateRange)}.
              </Text>
            )}
          </BlockStack>
        </Modal.Section>
