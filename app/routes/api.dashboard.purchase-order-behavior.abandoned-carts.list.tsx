...
/**
 * API Route for Abandoned Carts List Data
 * 
 * Returns orders that reached the payment page but didn't complete.
 * These are orders with PENDING, AUTHORIZED, or PARTIALLY_PAID status
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    while (hasNextPage && allOrders.length < 1000) {
      ...
      // 3. Are not cancelled (cancelled orders are tracked separately)
      const abandonedCarts = orders.filter(
        (o: any) =>
          ...
          !o.cancelledAt &&
          o.paymentGatewayNames &&
          o.paymentGatewayNames.length > 0
      );
      allOrders = [...allOrders, ...abandonedCarts];
    ...
    if (error.message === "PROTECTED_ORDER_DATA_ACCESS_DENIED") {
      console.log(
        "[Abandoned Carts List API] Protected order data access denied",
      );
      return Response.json(
        { error: "PROTECTED_ORDER_DATA_ACCESS_DENIED" },
        { status: 403 }
